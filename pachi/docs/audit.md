...
- **Actions**:
  1. Adopt a standard Godot 4 C# test framework (e.g. `GoDotTest` + `Shouldly` or `GdUnit4Net`).
  2. Migrate existing test suites (`BallStuckTests`, `BallVisualsTests`, `CardGuardrailTests`, `PocketIndicatorTests`, `PinToolPerformanceTests`) to discrete test classes using `[Test]` / `[TestCase]` with `[Setup]` / `[Cleanup]` lifecycles.
  3. Split test suites into two distinct categories:
     - **Pure Domain Tests**: Fast, POCO unit tests (deck costs, payout multipliers, ball queue logic) without `SceneTree` overhead.
     - **Scene Integration Tests**: Headless Godot node tests.
  4. Remove screenshot capture assertions from headless CI runs.
  5. Update `./scripts/verify.sh` to run the structured test suite headlessly.
