...
[GlobalClass]
public partial class DecrementOutputTierArchetype : CardArchetype
{
                ...
                if (idx > 0)
                {
                    ...
                    if (seen.Add(key))
                    {
                        options.Add(new ModifyOutputTierCardData
                        {
                            ...
                            Title = $"Out: T{idx + 1} -> T{idx}",
                            Description = $"Downgrade 1 Tier {idx + 1} output payout to Tier {idx}. Gives a Ball Pack to hopper!",
                            CardColor = new Color(0.65f, 0.4f, 0.35f),
